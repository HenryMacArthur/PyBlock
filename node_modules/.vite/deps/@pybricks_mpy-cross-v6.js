...
//#endregion
//#region node_modules/@pybricks/mpy-cross-v6/build/mpy-cross-v6.js
var require_mpy_cross_v6 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var MpyCross = (() => {
		var _scriptDir = typeof document !== "undefined" && document.currentScript ? document.currentScript.src : void 0;
		if (typeof __filename !== "undefined") _scriptDir = _scriptDir || __filename;
		return (function(MpyCross) {
			MpyCross = MpyCross || {};
			var Module = typeof MpyCross != "undefined" ? MpyCross : {};
			...
			const outputFileName = outputOptionFlagIndex >= 0 ? args[outputOptionFlagIndex + 1] : inputFileName.replace(".py", ".mpy");
			Module["preRun"] = (() => {
			...
			Module["onExit"] = ((status) => {
				const mpy = status === 0 ? FS.readFile(outputFileName, { encoding: "binary" }) : void 0;
				try {
			...
			if (ENVIRONMENT_IS_NODE) {
				if (ENVIRONMENT_IS_WORKER) scriptDirectory = require_browser_external_path().dirname(scriptDirectory) + "/";
				else scriptDirectory = __dirname + "/";
				requireNodeFS = (() => {
					if (!nodePath) {
						fs = require_browser_external_fs();
						nodePath = require_browser_external_path();
					}
				});
			...
			var wasmBinaryFile = "mpy-cross-v6.wasm";
			...
			function getBinaryPromise() {
				if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER)) {
					if (typeof fetch == "function" && !isFileURI(wasmBinaryFile)) return fetch(wasmBinaryFile, { credentials: "same-origin" }).then(function(response) {
						if (!response["ok"]) throw "failed to load wasm binary file at '" + wasmBinaryFile + "'";
						return response["arrayBuffer"]();
					}).catch(function() {
						return getBinary(wasmBinaryFile);
					});
					else if (readAsync) return new Promise(function(resolve, reject) {
			...
			function createWasm() {
				...
				function instantiateAsync() {
					if (!wasmBinary && typeof WebAssembly.instantiateStreaming == "function" && !isDataURI(wasmBinaryFile) && !isFileURI(wasmBinaryFile) && typeof fetch == "function") return fetch(wasmBinaryFile, { credentials: "same-origin" }).then(function(response) {
			...
			function getRandomDevice() {
				if (typeof crypto == "object" && typeof crypto["getRandomValues"] == "function") {
					...
					var crypto_module = require_browser_external_crypto();
					return function() {
			...
			var FS = {
				...
				lookupPath: (path, opts = {}) => {
					...
					for (var key in defaults) if (opts[key] === void 0) opts[key] = defaults[key];
					...
					for (var i = 0; i < parts.length; i++) {
						...
						if (!islast || opts.follow) {
							...
							while (FS.isLink(current.mode)) {
								...
								current = FS.lookupPath(current_path, { recurse_count: opts.recurse_count }).node;
			...
			function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
				try {
					var stream = SYSCALLS.getStreamFromFD(fd);
					var offset = offset_high * 4294967296 + (offset_low >>> 0);
			...
			var ___stdio_exit = Module["___stdio_exit"] = function() {
				return (___stdio_exit = Module["___stdio_exit"] = Module["asm"]["v"]).apply(null, arguments);
			};
			var ___funcs_on_exit = Module["___funcs_on_exit"] = function() {
				return (___funcs_on_exit = Module["___funcs_on_exit"] = Module["asm"]["w"]).apply(null, arguments);
			};
			...
			run();
			return MpyCross.ready;
		});
	})();
	if (typeof exports === "object" && typeof module === "object") module.exports = MpyCross;
	else if (typeof define === "function" && define["amd"]) define([], function() {
		return MpyCross;
	});
	else if (typeof exports === "object") exports["MpyCross"] = MpyCross;
}));
//#endregion
//#region node_modules/@pybricks/mpy-cross-v6/build/index.js
var require_build = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var MpyCross = __importDefault(require_mpy_cross_v6()).default;
	/**
	...
	*/
	function compile(fileName, fileContents, options, wasmPath) {
		return new Promise(function(resolve, reject) {
			try {
				var args = [fileName];
				if (options) args.splice.apply(args, __spreadArrays([0, 0], options));
				MpyCross({
					arguments: args,
					...
					locateFile: function(path, scriptDirectory) {
						if (path === "mpy-cross-v6.wasm" && wasmPath !== void 0) return wasmPath;
						return scriptDirectory + path;
					}
				});
			} catch (err) {
...
//# sourceMappingURL=@pybricks_mpy-cross-v6.js.map